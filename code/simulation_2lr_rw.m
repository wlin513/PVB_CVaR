...
for ns=1:nsim
    ...
    for i=1:length(alltypes)
        ...
        for j=1:length(alpha_space)
            for k=1:length(alpha_space)
             alpha=[alpha_space(j),alpha_space(k)];
             pos_bias(j,k)=alpha_space(j)/(alpha_space(j)+alpha_space(k));
             r.(blktype)(:,j,k)=rescorla_wagner_PNPE_2lr(data(:,1)',data(:,2)',alpha,start);
            end
        end
...
pos_bias=reshape(pos_bias,size(pos_bias,1)*size(pos_bias,2),1);
f4=figure;
for i=1:length(alltypes)
    blktype=alltypes{i};
    pp=squeeze(mean(mean(mean(pchoice.(blktype),1),2),5));
    pp=reshape(pp,size(pp,1)*size(pp,2),1);
    scatter(pos_bias,pp)
    hold on;
end
%legend(regexprep(alltypes,'_','-'),'Location','southeastoutside','AutoUpdate','off')
xlabel('positive bias in learning rates')
ylabel('probabilities of choosing the broader/higher option')
title(strrep(model,'_','-'))
fplot(@(x) 0.5,[0 1],'--','Color','k')
ylim([0.3 0.8])
...
for i=1:length(alltypes)
    blktype=alltypes{i};
    plot(squeeze(mean(mean(mean(mean(pchoice.(blktype),1),2),3),4)),'LineWidth',5)
    hold on;
end
