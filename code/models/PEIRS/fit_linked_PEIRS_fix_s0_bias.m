function [out]=fit_linked_PEIRS_fix_s0_bias(opt1_events,opt2_events,choice,startQ,startS,abandontn,resp_made,alphabins,betabins,omegabins,phibins,fig_yes)


% Bayesian model fit
if(nargin<12) fig_yes=0; end
if(nargin<11) phibins=30; end
...
phirange=0.5;
phis=-phirange:(phirange-(-phirange))/(phibins-1):phirange;
...
for q=1:length(o)
    for p=1:length(phis)
    q1(:,:,:,:,q,p)=v1+(tanh(o(q).*pes)+phis(p)).*s1;
    q2(:,:,:,:,q,p)=v2+(tanh(o(q).*pes)+phis(p)).*s2;
    end
end
...
beta=repmat(permute(exp(b_log),[1 3 4 5 6 7 2]),[length(i) length(i) size(opt1_events,1) size(opt2_events,2) length(o) length(phis) 1]);
...
% choice of a given participant
  ...
  ch=repmat(permute(choice,[3 4 1 2 5 6]),[length(i) length(i) 1 1 length(o) length(phis) length(b_log)]);
...
out.marg_omega=squeeze(sum(sum(sum(sum(out.posterior_prob,1),2),4),5));
out.mean_omega=o*out.marg_omega;
out.var_omega=(o-out.mean_omega).^2*out.marg_omega;
%phi
    out.marg_phi=squeeze(sum(sum(sum(sum(out.posterior_prob,1),2),3),5));
    out.mean_phi=phis*out.marg_phi;
    out.var_phi=(phis-out.mean_phi).^2*out.marg_phi;
...
pes=(v1+v2)./2-0.5;
bel=v1+(tanh(out.mean_omega.*pes)+out.mean_phi).*s1-v2-(tanh(out.mean_omega.*pes)+out.mean_phi).*s2;
...
out.BIC=(2.*out.neg_log_like)+5*(log(size(likelihood,1)*size(likelihood,2))-log(2*pi)); % BIC given 4 free parameters
