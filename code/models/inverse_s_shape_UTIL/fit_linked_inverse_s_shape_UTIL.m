function [out]=fit_linked_inverse_s_shape_UTIL(opt1_events,opt2_events,choice,start,abandontn,resp_made,alphabins,betabins,kbins,fig_yes)
...
% figure;
% for kk=1:5%length(k_space)
%     fplot(@(x) power_utility(x,exp(k_space(kk))),[0 1]);
%     hold on
% end
%   fplot(@(x) x,[0 1]);
...
for a=1:length(i)
    for kk=1:length(k_space)
        learn_left=rescorla_wagner_power_UTIL(opt1_events,opt2_events,inv_logit(i(a)),exp(k_space(kk)),start);
