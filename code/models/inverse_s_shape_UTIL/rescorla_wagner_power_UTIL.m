function [R]=rescorla_wagner_power_UTIL(opt1_events,opt2_events,alpha,k,start)
...
for b=1:size(opt1_events,1)
    for i=2:size(opt1_events,2)
        ...
        v1(b,i)=v1(b,i-1)+alpha*(power_utility(opt1_events(b,i-1),k)-v1(b,i-1));
        
        v2(b,i)=v2(b,i-1)+alpha*(power_utility(opt2_events(b,i-1),k)-v2(b,i-1));
    end
