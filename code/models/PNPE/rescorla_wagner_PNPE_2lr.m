function [R]=rescorla_wagner_PNPE_2lr(opt1_events,opt2_events,alpha,start)
...
v1(:,1)=start;
pe1(:,1)=opt1_events(:,1)-v1(:,1);
v2=zeros(size(opt2_events));
pe2=zeros(size(opt2_events));
v2(:,1)=start;
pe2(:,1)=opt2_events(:,1)-v2(:,1);
for b=1:size(opt1_events,1)
    for i=2:size(opt1_events,2)

        if pe1(b,i-1)>0
          v1(b,i)=v1(b,i-1)+alpha(1)*pe1(b,i-1);
        else 
              v1(b,i)=v1(b,i-1)+alpha(2)*pe1(b,i-1);
        end
        pe1(b,i)=opt1_events(b,i)-v1(b,i);
        
        if pe2(b,i-1)>0
          v2(b,i)=v2(b,i-1)+alpha(1)*pe2(b,i-1);
        else 
              v2(b,i)=v2(b,i-1)+alpha(2)*pe2(b,i-1);
        end
