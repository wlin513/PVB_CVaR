function [out]=fit_linked_distRL_LR_twoVars(opt1_events,opt2_events,choice,start,abandontn,resp_made,alpha_pos_bins,alpha_neg_bins,varsbins,fig_yes)
...
for i=1:length(alpha_pos_space)
    for j=1:length(alpha_neg_space)
        for k=1:length(vars_space)
            for q=1:length(vars_space)
                [expv1,expv2]=cal_EV_distRL_LR(alpha_pos_space(i),alpha_neg_space(j),vars_space(k),vars_space(q),opt1_events,opt2_events,start);
                for m=1:size(expv1,2)    
                    for l=abandontn+1:size(expv1,3)
                            probleft(i,j,k,q,m,l-abandontn)=read_prob(expv1(:,m,l),expv2(:,m,l));
                    end
                end
            end
        end
...
ch=repmat(permute(choice,[5 6 3 4 1 2]),[length(alpha_pos_space) length(alpha_neg_space) length(vars_space) length(vars_space) 1 1]);
...
clear probleft
  ...
  out.posterior_prob(:,:,:,:)=squeeze(prod(prod(probch,6),5))*10^(size(probch,6)*size(probch,5)/5);  
  %renormalise
...
out.marg_pos_alpha=squeeze(sum(sum(sum(out.posterior_prob,4),3),2));
...
out.marg_neg_alpha=squeeze(sum(sum(sum(out.posterior_prob,1),3),4))';
...
out.marg_vars1=squeeze(sum(sum(sum(out.posterior_prob,1),2),4));
out.mean_vars1=sqrt(s_vars_space*out.marg_vars1);
out.var_vars1=sqrt(((s_vars_space-(out.mean_vars1)).^4)*out.marg_vars1);

out.marg_vars2=squeeze(sum(sum(sum(out.posterior_prob,1),2),3));
out.mean_vars2=sqrt(s_vars_space*out.marg_vars2);
out.var_vars2=sqrt(((s_vars_space-(out.mean_vars2)).^4)*out.marg_vars2);
    

out.alpha_pos_label=alpha_pos_space;
out.vars_label=vars_space;
out.alpha_neg_label=alpha_neg_space;
out.betab_plr=(out.mean_pos_alpha - out.mean_vars1 + out.mean_pos_alpha.*out.mean_vars1 - out.mean_pos_alpha.^2*2 + out.mean_pos_alpha.^3)./out.mean_vars1;
out.betaa_plr=-(out.mean_pos_alpha.*(out.mean_pos_alpha.^2 - out.mean_pos_alpha + out.mean_vars1))./out.mean_vars1;

out.betab_nlr=(out.mean_neg_alpha - out.mean_vars2 + out.mean_neg_alpha.*out.mean_vars2 - out.mean_neg_alpha.^2*2 + out.mean_neg_alpha.^3)./out.mean_vars2;
out.betaa_nlr=-(out.mean_neg_alpha.*(out.mean_neg_alpha.^2 - out.mean_neg_alpha + out.mean_vars2))./out.mean_vars2;
...
if fig_yes==1 
   ...
   subplot(3,1,2);
   plot(vars_space,out.marg_vars1);
   title('variance');
   subplot(3,1,3);
...
[expv1,expv2]=cal_EV_distRL_LR(out.mean_pos_alpha,out.mean_neg_alpha,out.mean_vars1,out.mean_vars2,opt1_events,opt2_events,start,100);
for m=1:size(expv1,2)
    for l=abandontn+1:size(expv1,3)
...
out.BIC=(2.*out.neg_log_like)+4*(log(sum(sum(resp_made)))-log(2*pi)); % BIC given 3 free parameters
