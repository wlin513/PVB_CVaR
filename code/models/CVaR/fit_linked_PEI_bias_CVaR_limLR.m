function [out]=fit_linked_PEI_bias_CVaR(opt1_events,opt2_events,choice,start,abandontn,resp_made,omegabins,betabins,alphabins,zetabins,fig_yes)
%inputs opt1_events[nblk,ntrial],opt2_events[nblk,ntrial],choice[nblk,ntrial]
if(nargin<11)fig_yes=1; end
if(nargin<10)zetabins=30; end
...
logit_zeta_space=logit(0.01):(logit(0.99)-logit(0.01))/(zetabins-1):logit(0.99);
zeta_space=inv_logit(logit_zeta_space);
...
for a=1:length(alpha_space)
    ...
    for i=1:length(o)            
            for b=1:size(pes,1)
                for t=1:size(pes,2)
                    for z=1:length(zeta_space)
                        if pes(b,t)>0                        
                          eta=tanh(2*zeta(z)*o(i)*pes(b,t));
                        else
                            eta=tanh(2*(1-zeta(z))*o(i)*pes(b,t));
                        end
                        [cvar1(z,b,t),cvar2(z,b,t)]=read_cvar(eta,squeeze(opt1_expv(a,:,b,t))',squeeze(opt2_expv(a,:,b,t))');
                    end
                end
            end
...
ch=repmat(permute(choice,[3 4 5 6 1 2]),[length(alpha_space) length(o) length(beta_space) length(zeta_space) 1 1]);
...
out.marg_base_alpha=squeeze(sum(sum(sum(out.posterior_prob,4),3),2));
...
out.marg_omega=squeeze(sum(sum(sum(out.posterior_prob,1),3),4))';
...
out.marg_zeta=squeeze(sum(sum(sum(out.posterior_prob,1),3),2));
out.mean_zeta=inv_logit(logit_zeta_space*out.marg_zeta);
...
% get LL from estimated parameters
    ...
    for b=1:size(pes,1)
        for t=1:size(pes,2)
            if pes(b,t)>0
               eta=tanh(2*out.mean_zeta*out.mean_omega.*pes(b,t));
            else
                eta=tanh(2*(1-out.mean_zeta)*out.mean_omega*pes(b,t));
            end
        [cvar1(b,t),cvar2(b,t)]=read_cvar(eta,opt1_expv1(:,b,t),opt2_expv1(:,b,t));
        end
    end
    cvardiff=cvar1-cvar2;
