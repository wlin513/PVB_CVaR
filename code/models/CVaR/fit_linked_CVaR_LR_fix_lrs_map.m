function [out]=fit_linked_CVaR_LR_fix_lrs_map(opt1_events,opt2_events,choice,start,abandontn,resp_made,etabins,betabins,alphabins,fig_yes)
...
%
taus=0.001:0.001:0.999;
mlr=0.001:0.001:0.999;
for i=1:length(mlr)
    for j=1:length(taus)
        lrs(i,j,1)=taus(j).*mlr(i).*2;
        lrs(i,j,2)=(1-taus(j)).*mlr(i).*2;
        if lrs(i,j,1)>1|lrs(i,j,2)>1
            lrs(i,j,1)=lrs(i-1,j,1);
            lrs(i,j,2)=lrs(i-1,j,2);
        end       
    end
end
...
for a=1:length(alpha_space)
    alpha_pos=lrs(find(mlr==round(alpha_space(a),3)),:,1);
    alpha_neg=lrs(find(mlr==round(alpha_space(a),3)),:,2);
...
% get LL from estimated parameters
    ...
    alpha_pos=lrs(find(mlr==round(out.mean_base_alpha,3)),:,1);
    alpha_neg=lrs(find(mlr==round(out.mean_base_alpha,3)),:,2);
