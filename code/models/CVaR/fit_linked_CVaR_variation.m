function [out]=fit_linked_CVaR_variation(opt1_events,opt2_events,choice,start,abandontn,resp_made,etabins,betabins,varsbins,fig_yes)
...
pos_bias=0.5;
betab=(pos_bias - vars + pos_bias.*vars - pos_bias.^2*2 + pos_bias.^3)./vars;
betaa=-(pos_bias.*(pos_bias.^2 - pos_bias + vars))./vars;
        

    opt1_expv=ones(length(vars_space),nneuo,size(opt1_events,1),size(opt1_events,2))*start;
    opt2_expv=ones(length(vars_space),nneuo,size(opt2_events,1),size(opt2_events,2))*start;
    ...
    for k=1:size(opt1_events,1)
        for i=1:size(opt1_events,2)-1
            for j=1:length(alpha_pos)
                %learn first option
                if opt1_events(k,i)>opt1_expv(vv,j,k,i)
                 opt1_expv(vv,j,k,i+1)=opt1_expv(vv,j,k,i)+alpha_pos(j)*(opt1_events(k,i)-opt1_expv(vv,j,k,i));
                else 
                    opt1_expv(vv,j,k,i+1)=opt1_expv(vv,j,k,i)+alpha_neg(j)*(opt1_events(k,i)-opt1_expv(vv,j,k,i));
                end
                
                %learn the second option              
                if opt2_events(k,i)>opt2_expv(vv,j,k,i)
                    opt2_expv(vv,j,k,i+1)=opt2_expv(vv,j,k,i)+alpha_pos(j)*(opt2_events(k,i)-opt2_expv(vv,j,k,i));
                else
                    opt2_expv(vv,j,k,i+1)=opt2_expv(vv,j,k,i)+alpha_neg(j)*(opt2_events(k,i)-opt2_expv(vv,j,k,i));
                end
            end
        end
    end
...
for vv=1:length(vars)
    for i=1:length(eta_space)
        ...
        [cvar1,cvar2]=read_cvar(eta_space(i),squeeze(opt1_expv(vv,:,:,:)),squeeze(opt2_expv(vv,:,:,:)));
        cvardiff=cvar1-cvar2;
        prob1(vv,i,j,:,:)=1./(1+exp(-(beta_space(j).*cvardiff)));
        end
    end
end

ch=repmat(permute(choice,[3 4 5 1 2]),[length(vars_space) length(eta_space) length(beta_space) 1 1]);
...
out.marg_vars=squeeze(sum(sum(out.posterior_prob,2),3));
...
% get LL from estimated parameters
    ...
    opt2_expv1=ones(nneuo,size(opt2_events,1),size(opt2_events,2))*start;
    for k=1:size(opt1_events,1)
        for i=1:size(opt1_events,2)-1
            for j=1:length(alpha_pos)
                %learn first option
