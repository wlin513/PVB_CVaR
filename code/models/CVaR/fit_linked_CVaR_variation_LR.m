function [out]=fit_linked_CVaR_variation_LR(opt1_events,opt2_events,choice,start,abandontn,resp_made,etabins,betabins,varsbins,alphabins,fig_yes)
%inputs opt1_events[nblk,ntrial],opt2_events[nblk,ntrial],choice[nblk,ntrial]
if(nargin<11)fig_yes=1; end
if(nargin<10) alphabins=30; end
...
pos_bias=0.5;
betab=(pos_bias - vars_space + pos_bias.*vars_space - pos_bias.^2*2 + pos_bias.^3)./vars_space;
betaa=-(pos_bias.*(pos_bias.^2 - pos_bias + vars_space))./vars_space;
        

opt1_expv=ones(length(alpha_space),length(vars_space),nneuo,size(opt1_events,1),size(opt1_events,2))*start;
opt2_expv=ones(length(alpha_space),length(vars_space),nneuo,size(opt2_events,1),size(opt1_events,2))*start;

    
for a=1:length(alpha_space)
    for vv=1:length(vars_space)
        taus=betarnd(betaa(vv),betab(vv),nneuo,1);
        ...
        for k=1:size(opt1_events,1)
            for i=1:size(opt1_events,2)-1
                for j=1:length(alpha_pos)
                    %learn first option
                    if opt1_events(k,i)>opt1_expv(a,vv,j,k,i)
                     opt1_expv(a,vv,j,k,i+1)=opt1_expv(a,vv,j,k,i)+alpha_pos(j)*(opt1_events(k,i)-opt1_expv(a,vv,j,k,i));
                    else 
                        opt1_expv(a,vv,j,k,i+1)=opt1_expv(a,vv,j,k,i)+alpha_neg(j)*(opt1_events(k,i)-opt1_expv(a,vv,j,k,i));
                    end

                    %learn the second option              
                    if opt2_events(k,i)>opt2_expv(a,vv,j,k,i)
                        opt2_expv(a,vv,j,k,i+1)=opt2_expv(a,vv,j,k,i)+alpha_pos(j)*(opt2_events(k,i)-opt2_expv(a,vv,j,k,i));
                    else
                        opt2_expv(a,vv,j,k,i+1)=opt2_expv(a,vv,j,k,i)+alpha_neg(j)*(opt2_events(k,i)-opt2_expv(a,vv,j,k,i));
                    end
                end
            end
        end
...
for a=1:length(alpha_space)
    for vv=1:length(vars_space)
        for i=1:length(eta_space)
            ...
            [cvar1,cvar2]=read_cvar(eta_space(i),squeeze(opt1_expv(a,vv,:,:,:)),squeeze(opt2_expv(a,vv,:,:,:)));
            cvardiff=cvar1-cvar2;
            prob1(a,vv,i,j,:,:)=1./(1+exp(-(beta_space(j).*cvardiff)));
            end
        end
    end
end
ch=repmat(permute(choice,[3 4 5 6 1 2]),[length(alpha_space) length(vars_space) length(eta_space) length(beta_space) 1 1]);
...
out.marg_base_alpha=squeeze(sum(sum(sum(out.posterior_prob,3),2),4));
...
out.marg_eta=squeeze(sum(sum(sum(out.posterior_prob,1),2),4));
...
% get LL from estimated parameters
    betab1=(pos_bias - out.mean_vars + pos_bias.*out.mean_vars - pos_bias.^2*2 + pos_bias.^3)./out.mean_vars;
    betaa1=-(pos_bias.*(pos_bias.^2 - pos_bias + out.mean_vars))./out.mean_vars;
    taus=betarnd(betaa1,betab1,1000,1);
